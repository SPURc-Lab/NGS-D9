...
# 1. if there's Dt ncbi sequence, always at the top hit, then 
# 2. Best E Value (the lowest E Value)
# and the non top-hit Dt names would be generated in deleted_dtnames.txt
# note: before this program, rename the ncbi files from "gi|" to "Locus_0_gi|"
# after the run, in the output file (deleted_dtnames.txt), remove the names from ncbi databse (considering the sequences from ncbi are non-redundant)
...
def gene_model():
    ...
    for line in inFile.readlines():
            array.append([line.strip().split('\t')[0], line.strip().split('\t')[1], float(line.strip().split('\t')[2]), int(line.strip().split('\t')[0].split('_')[1])]) 

    array.sort(key=lambda x: (x[1], x[3], x[2]))
