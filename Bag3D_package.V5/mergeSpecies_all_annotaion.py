#!/usr/bin/python
# Usage mergeSpecies_all_annotaion.py <Original FASTA format file> <pacID file1> <pacID file2> <pacID file3> <pacID file4> <pacID file5>
# <Original FASTA format file> e.g. Dt_all_seq.fa
# <pacID1> --> <pacID5>: Cre_pacID.txt Csu_pacID.txt Olu_pacID.txt Vcar_pacID.txt Chorella_pacID.txt 
...
def delete_dtseq():
    ...
    outFile = open("merge_annotation_output.txt", 'w')
    ...
    while ptr_base < len(dt_base):
        ...
        if dt_base[ptr_base] == tr1[ptr1].split()[0]: 
            tmp_line = tmp_line + '\t' + tr1[ptr1].split()[1]
            if ptr1 < len(tr1) - 1:
                ptr1 = ptr1 + 1
        else:
            tmp_line = tmp_line + '\t'
        if dt_base[ptr_base] == tr2[ptr2].split()[0]: 
            tmp_line = tmp_line + '\t' + tr2[ptr2].split()[1]
            if ptr2 < len(tr2) - 1:
                ptr2 = ptr2 + 1
        else:
            tmp_line = tmp_line + '\t'
        ...
        if dt_base[ptr_base] == tr3[ptr3].split()[0]:
            tmp_line = tmp_line + '\t' + tr3[ptr3].split()[1]
            if ptr3 < len(tr3) - 1:
                ptr3 = ptr3 + 1
        else:
            tmp_line = tmp_line + '\t'
